...
class ConfigValidator:
    ...
    def validate_url(self, url: str, name: str, require_https: bool = True) -> bool:
        ...
        try:
            ...
            if len(url) > 2048:
                self.errors.append(
                    f"{name}: URL exceeds maximum length of 2048 characters"
                )
                return False
    ...
    def validate_port(self, port: str, name: str) -> bool:
        ...
        try:
            ...
            if port_num < 1 or port_num > 65535:
                self.errors.append(
                    f"{name}: Port must be between 1 and 65535, got {port_num}"
                )
                return False
            return True
        except ValueError:
    ...
    def validate_interval(self, interval: str, name: str, min_val: int = 60) -> bool:
        ...
        try:
            ...
            if interval_num < min_val:
                self.errors.append(
                    f"{name}: Interval must be at least {min_val} seconds, got {interval_num}"
                )
                return False
            return True
        except ValueError:
    ...
    def validate_telegram_token(self, token: str) -> bool:
        ...
        if not re.match(token_pattern, token):
            self.errors.append(
                "TELEGRAM_BOT_TOKEN: Invalid format (should be like 123456789:ABCdefGHI...)"
            )
            return False
    ...
    def validate_discord_config(
        self, enabled: str, webhook_url: str, avatar_url: str
    ) -> bool:
        ...
        if enabled.lower() == "true":
            ...
            if not self.validate_url(
                webhook_url, "DISCORD_WEBHOOK_URL", require_https=True
            ):
                return False
            ...
            if avatar_url and not self.validate_url(
                avatar_url, "DISCORD_AVATAR_URL", require_https=False
            ):
                return False
    ...
    def validate_telegram_config(
        self, enabled: str, bot_token: str, chat_id: str, parse_mode: str
    ) -> bool:
        ...
        if enabled.lower() == "true":
            ...
            if parse_mode not in ["HTML", "Markdown", "MarkdownV2"]:
                self.warnings.append(
                    f"TELEGRAM_PARSE_MODE: Unknown mode '{parse_mode}', use HTML or Markdown"
                )
    ...
    def validate_email_config(
        ...
        if enabled.lower() == "true":
            ...
            for field_name, field_value in required_fields.items():
                if not field_value:
                    self.errors.append(
                        f"EMAIL_ENABLED is true but {field_name} is not set"
                    )
                    return False
    ...
    def validate_general_config(
        self, check_interval: str, monitor_ipv4: str, monitor_ipv6: str
    ) -> bool:
        ...
        try:
            if int(check_interval) < 300:  # 5 minutes
                self.warnings.append(
                    "CHECK_INTERVAL is less than 5 minutes - may cause excessive API calls"
                )
        except ValueError:
            pass
        ...
        if monitor_ipv4.lower() == "false" and monitor_ipv6.lower() == "false":
            self.errors.append(
                "Both MONITOR_IPV4 and MONITOR_IPV6 are disabled - at least one must be enabled"
            )
            return False
    ...
    def validate_update_config(
        self, enabled: str, interval: str, on_startup: str
    ) -> bool:
        ...
        if enabled.lower() == "true":
            ...
            if not self.validate_interval(
                interval, "UPDATE_CHECK_INTERVAL", min_val=3600
            ):
                return False
    ...
    def validate_all(self) -> Tuple[bool, List[str], List[str]]:
        ...
        self.validate_telegram_config(
            telegram_enabled, telegram_token, telegram_chat_id, telegram_parse_mode
        )
        ...
        if (
            discord_enabled.lower() == "false"
            and telegram_enabled.lower() == "false"
            and email_enabled.lower() == "false"
        ):
            self.errors.append(
        ...
        if email_enabled.lower() == "true" and all(
        ...
        if not platforms_enabled:
            self.errors.append(
                "No notification platforms properly configured - check your settings"
            )
