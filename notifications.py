    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                delay = base_delay * (2**attempt)
                logger.warning(
                    f"Attempt {attempt + 1} failed with error: {e}, retrying in {delay}s..."
                )
                time.sleep(delay)
            else:
...
class NotificationProvider:
    ...
    def send_update_notification(
        self, update_info: Dict[str, str], server_name: str, version: str = "1.4.0"
    ) -> bool:
...
class DiscordNotifier(NotificationProvider):
        ...
        try:
            ...
            if is_first_run:
                ...
                if current_ips.get("ipv4") != previous_ips.get("ipv4"):
                    changes.append(
                        f"**IPv4:** `{previous_ips.get('ipv4', 'None')}` → `{current_ips.get('ipv4', 'None')}`"
                    )
                if current_ips.get("ipv6") != previous_ips.get("ipv6"):
                    changes.append(
                        f"**IPv6:** `{previous_ips.get('ipv6', 'None')}` → `{current_ips.get('ipv6', 'None')}`"
                    )
            ...
            if response.status_code == 204:
                logger.info(
                    f"Discord notification sent successfully (Status: {response.status_code})"
                )
                return True
            else:
                logger.error(
    ...
    def send_update_notification(
        ...
        try:
            ...
            payload = {"username": self.bot_name, "embeds": [embed]}
            ...
            if response.status_code == 204:
                ...
                logger.error(
                    f"Discord update notification failed (Status: {response.status_code})"
                )
                return False
    ...
    def send_notification(
        ...
        try:
            ...
            if not is_first_run:
                ...
                if current_ips.get("ipv4") != previous_ips.get("ipv4"):
                    message_lines.append(
                        f"  • IPv4: <code>{previous_ips.get('ipv4', 'None')}</code> → <code>{current_ips.get('ipv4', 'None')}</code>"
                    )
                if current_ips.get("ipv6") != previous_ips.get("ipv6"):
                    message_lines.append(
                        f"  • IPv6: <code>{previous_ips.get('ipv6', 'None')}</code> → <code>{current_ips.get('ipv6', 'None')}</code>"
                    )
                message_lines.append("")
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Telegram notification failed (Status: {response.status_code}): {response.text}"
                )
                return False
    ...
    def send_update_notification(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Telegram update notification failed (Status: {response.status_code})"
                )
                return False
...
class EmailNotifier(NotificationProvider):
    ...
    def _build_html_email(
        ...
        if is_first_run:
            ...
            if current_ips.get("ipv4") != previous_ips.get("ipv4"):
                changes.append(
                    f"<strong>IPv4:</strong> {previous_ips.get('ipv4', 'None')} → {current_ips.get('ipv4', 'None')}"
                )
            if current_ips.get("ipv6") != previous_ips.get("ipv6"):
                changes.append(
                    f"<strong>IPv6:</strong> {previous_ips.get('ipv6', 'None')} → {current_ips.get('ipv6', 'None')}"
                )
...
"""
    ...
    def send_notification(
        ...
        try:
            ...
            logger.info(
                f"Email notification sent successfully to {', '.join(self.to_addrs)}"
            )
            return True
...
"""
            ...
            logger.info(
                f"Email update notification sent successfully to {', '.join(self.to_addrs)}"
            )
            return True
...
class NotificationManager:
    ...
    def notify_update(
        ...
        for provider in self.providers:
            ...
            # Create a lambda that wraps the provider call
            def send_func():
                return provider.send_update_notification(
                    update_info, server_name, version
                )
            ...
            if success:
                ...
                logger.error(
                    f"{provider_name} update notification failed after all retries"
                )
